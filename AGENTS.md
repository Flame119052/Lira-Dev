...
- **Plan reviewed by a cheap, different model before coding.** After branching but before the first commit, draft a short implementation plan (approach, files/modules to touch, test seam, how each acceptance criterion will be met) and spawn **one** reviewer thread with a *different*, cheap model to poke holes in it. Cheap pool (all via `acp-opencode2`): `opencode-go/muse-spark-1.2-contributor`, `opencode-go/deepseek-v4-flash`, `opencode-go/ox-alpha-free` — pick whichever is not the implementer's own family (e.g. Muse Spark implements → Ox Alpha or DeepSeek reviews; Grok/Codex implements → Muse Spark reviews). The reviewer checks for overengineering, missing AC coverage, and ticket-DAG blast-radius blind spots; the implementer addresses or explicitly rejects each point before writing code. One round, fast — this is a cheap pre-check, not the four-model audit.
...
| Grok 4.6 | `acp-cursor` | `cursor-grok-4.6-medium` (`--reasoning-level high`) |
